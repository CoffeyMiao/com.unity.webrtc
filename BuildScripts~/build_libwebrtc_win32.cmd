...
rem add jsoncpp
rem outdated, add it manually to avoid link errors
rem LNK2019: unresolved external symbol "public: class Json::ValueIterator __thiscall Json::Value::end(void)"
@REM patch -N "src\BUILD.gn" < "%COMMAND_DIR%\patches\add_jsoncpp.patch"

rem install pywin32
@REM call "%cd%\depot_tools\bootstrap-3_8_0_chromium_8_bin\python\bin\python.exe" ^
@REM   -m pip install pywin32 --index-url "%PYPI_URL%" --upgrade
call "%cd%\depot_tools\bootstrap-2@3_8_10_chromium_26_bin\python\bin\python.exe" ^
...
rem no need to generate license, just copy one manually

@REM rem fix error when generate license
@REM patch -N "%cd%\src\tools_webrtc\libs\generate_licenses.py" < ^
@REM   "%COMMAND_DIR%\patches\generate_licenses.patch"

@REM rem generate license
@REM call python.bat "%cd%\src\tools_webrtc\libs\generate_licenses.py" ^
@REM   --target //:default %OUTPUT_DIR% %OUTPUT_DIR%

@REM rem unescape license
@REM powershell -File "%COMMAND_DIR%\Unescape.ps1" "%OUTPUT_DIR%\LICENSE.md"

@REM rem copy header
@REM xcopy src\*.h "%ARTIFACTS_DIR%\include" /C /S /I /F /H

@REM rem copy license
@REM copy "%OUTPUT_DIR%\LICENSE.md" "%ARTIFACTS_DIR%"
